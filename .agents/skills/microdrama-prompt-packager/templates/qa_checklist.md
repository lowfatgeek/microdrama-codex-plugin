# Model-Aware QA Checklist

- [ ] Every recurring main character has an approved GPT Image 2.5 sheet, or a visible fallback warning.
- [ ] Selected profile is recorded.
- [ ] Production specification records episode count, every episode duration, profile, unit plan, and calculated totals.
- [ ] Every episode duration is a positive multiple of 15 seconds.
- [ ] `legacy-15s` / `controlled-15s` has `duration ÷ 15` units per episode.
- [ ] `long-form-30s` uses maximum 30s units and one final 15s unit if needed.
